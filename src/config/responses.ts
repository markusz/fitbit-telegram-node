/* eslint-disable no-unused-vars,no-mixed-operators,max-len */
import { startCase } from 'lodash';

import { FitBitFoodIds, FitBitUnitIds } from './constants';
...
function numberAsStringToInt(numberAsString: string, multiplyWith = 1) {
...
const Responses: Response[] = [
  ...
  },
  {
    ...
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1])],
  },
  {
    meta: [/^p (\d+)$/i, Actions.LOG_CALORIES, 'Classic Ceasars Salad from Dean and David'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]) * 25, `${numberAsStringToInt(matchResult[1])} x Plätzchen`],
  },
  {
    meta: [/^dd c$/i, Actions.LOG_CALORIES, 'Classic Ceasars Salad from Dean and David'],
    getLogSpecifics: (matchResult: string[]) => [367 + 247 + 122, 'Classic Ceasars Salad (Dean & David)'],
  },
  {
    meta: [/^käse (\d+)$/i, Actions.LOG_FOOD, 'Eine Scheibe Bergbauernkäse mit etwa 22g'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 22), FitBitFoodIds.Kaese.KAESE],
  },
  {
    meta: [/^reis (\d+)$/i, Actions.LOG_FOOD, 'Gekochter Reis'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]), FitBitFoodIds.REIS, FitBitUnitIds.GRAMM],
  },
  {
  ...
  },
  {
    ...
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]), FitBitFoodIds.Vegetables.KARTOFFELN_GEKOCHT, FitBitUnitIds.GRAMM],
  },
  {
    meta: [/^nudeln (\d+)$/i, Actions.LOG_FOOD, 'Nudeln gekocht'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]), FitBitFoodIds.NUDELN, FitBitUnitIds.GRAMM],
  },
  {
    meta: [/^schinken (\d+)$/i, Actions.LOG_FOOD, 'Eine Scheibe Schinken mit etwa 20g'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 20), FitBitFoodIds.Wurst.SCHINKEN],
  },
  {
    meta: [/^salami s (\d+)$/i, Actions.LOG_FOOD, 'Kleine Salamischeibe mit etwa 4g (fein geschnitten, 4-5 cm im Durchmesser)'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 4), FitBitFoodIds.Wurst.SALAMI],
  },
  {
    meta: [/^salami m (\d+)$/i, Actions.LOG_FOOD, 'Mittlere Salamischeibe mit etwa 8g (etwas dicker geschnitten / größerer Durchmesser)'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 8), FitBitFoodIds.Wurst.SALAMI],
  },
  {
    meta: [/^salami l (\d+)$/i, Actions.LOG_FOOD, 'Große Salamischeibe mit etwa 12g (dick geschnitten, >8cm Durchmesser'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 12), FitBitFoodIds.Wurst.SALAMI],
  },
  {
    meta: [/^traube (\d+)$/i, Actions.LOG_FOOD, 'Trauben'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 4.3), FitBitFoodIds.Fruit.GRAPE],
  },
  {
  ...
  },
  {
    ...
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]), FitBitFoodIds.BUTTER, FitBitUnitIds.GRAMM],
  },
  {
  ...
  },
  {
    ...
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1], 22), FitBitFoodIds.Backwaren.KRAEUTERBAGUETTE],
  },
  {
    meta: [/^baguette (\d+)$/i, Actions.LOG_FOOD, 'Scheibe Baguette (20g)'],
    getLogSpecifics: (matchResult: string[]) => [numberAsStringToInt(matchResult[1]), 20, FitBitFoodIds.Backwaren.BAGUETTE],
  },
  {
